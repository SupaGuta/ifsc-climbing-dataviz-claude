...
- **`seq` semantics vary by discipline** — same column, different meanings:
  ...
  - **Speed-final heats**: `seq = heat_id` (the World Climbing heat identifier, monotonically allocated). One row per physical heat — eight 1/8 heats, four 1/4 heats, etc. heat_ids are large integers (e.g. 77865), so seq values for speed-final are much bigger than for other disciplines. Order by seq still yields chronological bracket order because World Climbing allocates heat_ids in time order.
...
- **Legacy / unknown speed heat names** that arrive without a `heat_id` fall back to `_speed_seq(name)` in [`fetchers/competitions.py`](https://github.com/SupaGuta/world-climbing-lab/blob/main/src/wcl_data/fetchers/competitions.py). `SPEED_HEAT_SEQ` maps known bracket names — canonical (`1/8`, `1/4`, `1/2`, `Small Final`, `Final`), localised (FR `Finale`, DE `Kleines Finale`), suffixed (`1/N - Final`, `1/N-Final`), dash-instead-of-slash (`1-N - Final`), and synonyms (`Semifinal` = 1/2, `sm. Final` = Small Final) — to seq 0-4. Lookup is case-insensitive and strips whitespace. Names that don't match any alias get seq=999. A warning is logged on every fallback (so you know which competitions used the heuristic, recognized alias or not). Audit periodically with:
  ```sql
